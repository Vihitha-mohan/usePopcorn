...
setWatched(watched=>watched.filter(movie=>movie.imdbID !==id))
  ...
  return (
    ...
    <Logo />
     ...
     <Main> 
     <Box >
      {/* {loader ? <Loading /> : <MovieList movies={movies} />} */}
      {loader && <Loading />}
...
<section>
  ...
  {userRating >0 && <button className="btn-add" onClick={handleAdd}>+Add to list</button>}</> : <p>Already added to list. Rating: {watchedUserRating}<span>⭐</span></p>}
  </div>
  <p><em>{plot}</em></p>
  <p>Staring : {actors}</p>
